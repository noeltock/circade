...
shas=$(git log --since="$since" --format=%H --reverse | awk -v n="$points" '{a[NR]=$0} END{if(NR==0)exit; for(i=1;i<=n;i++){idx=int(1+(NR-1)*(i-1)/(n-1+(n==1))); print a[idx]}}' | awk '!s[$0]++')
...
tmp=$(mktemp -d)
for s in $shas; do
  d="$tmp/$s"; mkdir -p "$d"; { git archive "$s" -- "${paths[@]}" 2>/dev/null; for op in ${oldpaths[@]+"${oldpaths[@]}"}; do git archive "$s" -- "$op" 2>/dev/null; done; } | tar -x -C "$d" 2>/dev/null; [ -n "$(ls -A "$d")" ] || { printf "%-10s %-10s  (none of the paths exist at this commit)\n" "$(git log -1 --format=%as $s)" "${s:0:8}"; continue; }
  G=(-g '!node_modules' -g '!vendor' -g '!dist' -g '!build' -g '!*.min.*' -g '!*.lock' -g '!package-lock.json')
  files=$(rg --files "${G[@]}" -g '*.{php,ts,tsx,js,jsx,py}' "$d" | wc -l | tr -d ' ')
  test_loc=$(rg --files "${G[@]}" -g '*.{php,ts,tsx,js,jsx,py}' "$d" | rg -i '(^|/)(tests?|__tests__|spec)/|\.(test|spec)\.' | xargs -I{} cat {} 2>/dev/null | wc -l | tr -d ' ')
  all_loc=$(rg --files "${G[@]}" -g '*.{php,ts,tsx,js,jsx,py}' "$d" | xargs -I{} cat {} 2>/dev/null | wc -l | tr -d ' ')
  src_loc=$((all_loc - test_loc))
  fns=$(rg -c "${G[@]}" -g '*.{php,ts,tsx,js,jsx,py}' -e '^\s*(export\s+)?(async\s+)?(public|protected|private|static|abstract|final)?\s*(function\s+\w+|def\s+\w+|const\s+\w+\s*=\s*(async\s*)?\(.*\)\s*=>)' "$d" | awk -F: '{s+=$NF}END{print s+0}')
  exports=$(rg -c "${G[@]}" -g '*.{ts,tsx,js,jsx}' -e '^\s*export\s|^\s*module\.exports\s*=|^\s*exports\.\w+\s*=' "$d" | awk -F: '{s+=$NF}END{print s+0}')
  imports=$(rg -c "${G[@]}" -g '*.{ts,tsx,js,jsx,php,py}' -e '^\s*(import\s|use\s+[A-Z]|from\s+\S+\s+import|require\s*\()' "$d" | awk -F: '{s+=$NF}END{print s+0}')
  ...
  printf "%-10s %-10s %7s %7s %7s %7s %7s %7s %7s %7s\n" "$(git log -1 --format=%as $s)" "${s:0:8}" "$files" "$src_loc" "$test_loc" "$ts" "$fns" "$exports" "$imports" "$dup"
done
rm -rf "$tmp"
